    ...
    <section className="py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <p className="mx-auto mt-3 max-w-2xl text-center text-neutral-500">
          Kompletní portfolio regulační techniky od světových výrobců
        </p>
        <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {CATEGORIES.map((cat) => (
            <Link
              key={cat.href}
              href={cat.href}
              className="group relative rounded-lg border border-neutral-200 border-l-4 border-l-transparent bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-l-dt-blue hover:shadow-md"
            >
              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-dt-blue/10 text-dt-blue">
                {cat.icon}
              </div>
              ...
              <span className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-dt-blue opacity-0 transition-opacity group-hover:opacity-100">
                Zobrazit
